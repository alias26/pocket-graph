"""tree -- emit a D3 v7 collapsible-tree HTML view of a pocket-graph graph.json.

A self-contained printable / browseable tree-of-modules view that
complements the force-directed ``graph.html`` produced by ``ingest``.
Key visual elements:
...
CLI: ``pocket-graph tree [--graph PATH] [--output HTML] [--root PATH]
...
# ââ Tree builder (filesystem hierarchy â JSON) ââââââââââââââââââ
    ...
    *,
    ...
    for src_file, syms in sorted(by_file.items()):
        ...
        for n in syms:
            ...
            # Skip the redundant file-name node from extraction.
...
# ââ HTML emitter (single-data-blob substitution) ââââââââââââââââââ
...
def write_tree_html(
    ...
    *,
    ...
    title = f"{tree['name']} -- pocket-graph tree viewer"
